...
void step1::Loop(TString inTreeName, TString outTreeName, const BTagCalibrationForLJMet* calib = NULL, const BTagCalibrationForLJMet* calib_dj = NULL)
{
  ...
  if (calib == NULL)
  {
    ...
    if ( Year == 2018 ) {
      btagcsvfile = "DeepCSV_106XUL18SF_V1p1.csv"; 
    }
	  if ( Year == 2016 ) {
      btagcsvfile = "DeepCSV_106XUL16SF.csv"; 
    }
  ...
  if ( calib_dj == NULL )
  {
    ...
    if ( Year == 2018 ) {
      btagdjcsvfile( "DeepJet_106XUL18SF_V1p1.csv" ); 
    }
    if ( Year == 2016 ) {
      btagdjcsvfile( "DeepJet_106XUL16SF.csv" ) 
    }
  ...
  if(inputTree->GetEntries()==0) {
    std::cout<<"[WARN] Found 0 events in the tree 'ljmet'"<<std::endl;
    return;
  }
  Init(inputTree);
...
// OUTPUT FILE
  ...
  for (Long64_t jentry=0; jentry<nentries;jentry++) {
    ...
    isElectron = 0;
    isMuon = 0;
    if(elPt_MultiLepCalc->size()==0 && muPt_MultiLepCalc->size()==0){std::cout << "[ERR] Got no leptons, something wrong." << std::endl; continue;}
    if(elPt_MultiLepCalc->size()>0 && muPt_MultiLepCalc->size()>0) std::cout << "[ERR] Got el and mu, something wrong." << std::endl;
    if(elPt_MultiLepCalc->size()>1 || muPt_MultiLepCalc->size()>1) std::cout << "[ERR] Got more than one el or mu, something wrong." << std::endl;
    ...
    if(isElectron==0 && isMuon==0){std::cout << "[ERR] Got no leptons, something wrong." << std::endl; continue;}
...
//       	}
    ...
    if(isMC){
      ...
      try {
        ...
        std::cout << "[WARN] I don't know this pileup sample, using TTToSemiLeptonic's: ";
    ...
    if (isMC) btagCSVRenormWeight = hardcodedConditions.GetCSVRenormSF(Year, isElectron, NJets_JetSubCalc, sampleType);
    ...
    for(unsigned int ijet=0; ijet < theJetPt_JetSubCalc->size(); ijet++){
      ...
      if (abs(ijetHFlv) ==5) { 
        ...
        djetWgt = reader_dj.eval_auto_bounds("central", BTagEntryForLJMet::FLAV_UDSG, jetaForBtag, jptForBtag, deepjet);
    ...
    // event selection ends, everything beyond here are calculations for passing events    
    ...
    for(unsigned int ijet=0; ijet < theJetPt_JetSubCalc_PtOrdered.size(); ijet++){
      ...
      if(theJetDeepFlavB_JetSubCalc_PtOrdered.at(ijet) > btagWPdjet) NJetsCSV_JetSubCalc += 1;
      if(theJetBTag_JetSubCalc_PtOrdered.at(ijet) == 1) NJetsCSVwithSF_JetSubCalc += 1;
      if(theJetBTag_bSFup_JetSubCalc_PtOrdered.at(ijet) == 1) NJetsCSVwithSF_JetSubCalc_bSFup += 1;
      if(theJetBTag_bSFdn_JetSubCalc_PtOrdered.at(ijet) == 1) NJetsCSVwithSF_JetSubCalc_bSFdn += 1;
      if(theJetBTag_lSFup_JetSubCalc_PtOrdered.at(ijet) == 1) NJetsCSVwithSF_JetSubCalc_lSFup += 1;
      if(theJetBTag_lSFdn_JetSubCalc_PtOrdered.at(ijet) == 1) NJetsCSVwithSF_JetSubCalc_lSFdn += 1;
