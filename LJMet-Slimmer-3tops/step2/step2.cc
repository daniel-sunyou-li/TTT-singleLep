...
TLorentzVector bestJet( 
  ...
  for(unsigned int i = 0; i < theJetPt_PtOrdered->size(); i++){
    jet_lv.SetPtEtaPhiE(
      theJetPt_PtOrdered->at(i),
      theJetEta_PtOrdered->at(i),
      theJetPhi_PtOrdered->at(i),
      theJetEnergy_PtOrdered->at(i)
    );
  ...
  jet_lv.SetPtEtaPhiE(
    theJetPt_PtOrdered->at(topIndex),
    theJetEta_PtOrdered->at(topIndex),
    theJetPhi_PtOrdered->at(topIndex),
    theJetEnergy_PtOrdered->at(topIndex)
  );
  return jet_lv;
}
...
{   
  ...
  // HT without best jet used in leptonic top reconstruction
  ...
  float btagWPdjet = hardcodedConditions.GetBTagWP( Year, "deepJet" ); // deepJet medium
  float btagWPdcsv = hardcodedConditions.GetBTagWP( Year, "deepCSV" ); // deepCSV medium

  cout << ">> Working points: " << endl << "  - deepJet: " << btagWPdjet << endl << "  - deepCSV: " << btagWPdcsv << endl;
  ...
  std::cout << ">> Running sample type: " << sampleType << std::endl;

     
  for ( Long64_t jentry = 0; jentry < nentries; jentry++ ){ // nentries
    // if ( jentry == 317626 ) continue; // need to identify bad events
    ...
    if( coin < 0.40 ) isTraining = 1;                               // TRAINING
    else if( ( coin >= 0.40 ) && ( coin < 0.60 ) ) isTraining = 2;  // TESTING
    else if( ( coin >= 0.60 ) && ( coin <= 1.00 ) ) isTraining = 3; // APPLICATION
    minBBdr = 1e9;
    minMleppJet = 1e9;     
    minMleppBjet = 1e9; // this doesn't get saved      
    aveCSVpt = 0;
    firstcsvb_bb = 0;
    secondcsvb_bb = 0;
    thirdcsvb_bb = 0;
    ...
    for(unsigned int i = 4; i < theJetPt_JetSubCalc_PtOrdered->size(); i++){
      HT_other += theJetPt_JetSubCalc_PtOrdered->at(i);
    }
    ...
    double lepM;
    double metM = 0;
    if(isMuon) lepM = 0.105658367; // lepton mass
    ...
    //  if( ( theJetDeepFlavB_JetSubCalc_PtOrdered->at(i) > btagWPdjet ) && ( theJetHFlav_JetSubCalc_PtOrdered->at(ijet) == 5 ) ) jettype = 1;
    //  else if ( ( theJetDeepFlavB_JetSubCalc_PtOrdered->at(i) > btagWPdjet ) && ( theJetHFlav_JetSubCalc_PtOrdered->at(ijet) != 5 ) ) jettype = 2;
    //  else if ( ( theJetDeepFlavB_JetSubCalc_PtOrdered->at(i) < btagWPdjet ) && ( theJetHFlav_JetSubCalc_PtOrdered->at(ijet) == 5 ) ) jettype = 3;
    //  else if ( ( theJetDeepFlavB_JetSubCalc_PtOrdered->at(i) < btagWPdjet ) && ( theJetHFlav_JetSubCalc_PtOrdered->at(ijet) != 5) ) jettype = 4;
      if( theJetDeepFlavB_JetSubCalc_PtOrdered->at(i) > btagWPdjet ){
        njetscsv+=1;
        totalPtCSV += theJetPt_JetSubCalc_PtOrdered->at(i); // used to calculate aveCSVpt later on 
        aveCSV += AK4JetDeepFlavb_MultiLepCalc_PtOrdered->at(i) + AK4JetDeepFlavbb_MultiLepCalc_PtOrdered->at(i) + AK4JetDeepFlavlepb_MultiLepCalc_PtOrdered->at(i);
        aveCSVpt += ( AK4JetDeepFlavb_MultiLepCalc_PtOrdered->at(i) + AK4JetDeepFlavbb_MultiLepCalc_PtOrdered->at(i) + AK4JetDeepFlavlepb_MultiLepCalc_PtOrdered->at(i) ) * theJetPt_JetSubCalc_PtOrdered->at(i);
      }

      if( i == 1 ) secondJetPt = theJetPt_JetSubCalc_PtOrdered->at( i );
      if( i == 4 ) fifthJetPt = theJetPt_JetSubCalc_PtOrdered->at( i );
      if( i == 5 ) sixthJetPt = theJetPt_JetSubCalc_PtOrdered->at( i );
      ...
      v_pair_jet_CSV.push_back(
        make_pair(
          jetTmp, 
          theJetDeepFlavB_JetSubCalc_PtOrdered->at(i)
        ) 
      ); 
      ...
      if( theJetDeepFlavB_JetSubCalc_PtOrdered->at(i) > btagWPdjet ){
        if( (lep + jetTmp ).M() < minMleppBjet) {
          minMleppBjet = fabs( ( lep + jetTmp ).M() );
      ...
      bool stop = 0;
      if ( tmpJetInd == 0 ){ 
        bjetTmp.SetPtEtaPhiE(
          theJetPt_JetSubCalc_PtOrdered->at(tmpJetInd),
          theJetEta_JetSubCalc_PtOrdered->at(tmpJetInd),
          theJetPhi_JetSubCalc_PtOrdered->at(tmpJetInd),
          theJetEnergy_JetSubCalc_PtOrdered->at(tmpJetInd)
        ); 
        for( unsigned int j = 1; j < theJetPt_JetSubCalc_PtOrdered->size(); j++ ){        
          if( ( theJetDeepFlavB_JetSubCalc_PtOrdered->at(j) > btagWPdjet ) && stop == 0 ){ 
            stop = 1;
            bjetTmp.SetPtEtaPhiE(
      ...
      if( tmpJetInd > 0 ){        
        stop = 0;
        for(unsigned int j = 0; j < tmpJetInd; j++){        
          if( ( theJetDeepFlavB_JetSubCalc_PtOrdered->at( j ) > btagWPdjet ) && stop == 0 ){
            stop = 1;
            bjetTmp.SetPtEtaPhiE(
              theJetPt_JetSubCalc_PtOrdered->at(j),
        ...
        if( stop == 0 ){
          for( unsigned int j = tmpJetInd + 1; j < theJetPt_JetSubCalc_PtOrdered->size(); j++){        
            if( ( theJetDeepFlavB_JetSubCalc_PtOrdered->at(j) > btagWPdjet ) && stop == 0 ){
              stop = 1;
              bjetTmp.SetPtEtaPhiE(
                theJetPt_JetSubCalc_PtOrdered->at(j),
      ...
      v_DCSV_allJets.push_back( theJetDeepFlavB_JetSubCalc_PtOrdered->at( i ) );
      if( i == 0 ) csvJet1 = theJetDeepFlavB_JetSubCalc_PtOrdered->at( i );
      else if( i == 1 ) csvJet2 = theJetDeepFlavB_JetSubCalc_PtOrdered->at( i );
      else if( i == 2 ) csvJet3 = theJetDeepFlavB_JetSubCalc_PtOrdered->at( i );
      else if( i == 3 ) csvJet4 = theJetDeepFlavB_JetSubCalc_PtOrdered->at( i );

      totalJetPt += theJetPt_JetSubCalc_PtOrdered->at(i);
      totalJetE += theJetEnergy_JetSubCalc_PtOrdered->at(i);
      deltaPhifromMET_ = TVector2::Phi_mpi_pi( theJetPhi_JetSubCalc_PtOrdered->at(i) - corr_met_phi_MultiLepCalc );
      ...
      if( abs( deltaPhifromMET_ ) > TMath::Pi()/2 ) hemiout += theJetPt_JetSubCalc_PtOrdered->at(i);				
...
// HT without the best jet where best jet is the one used in leptonic top reconstruction
    bestJetinWjet = bestJet( 
      corr_met_phi_MultiLepCalc, corr_met_MultiLepCalc,
      lepM, lep, 
      theJetPt_JetSubCalc_PtOrdered, 
      theJetEta_JetSubCalc_PtOrdered, 
      theJetPhi_JetSubCalc_PtOrdered, 
      theJetEnergy_JetSubCalc_PtOrdered
    );
...
// trijet selection 
    ...
    if ( isElectron == 1 && elNumberOfMothers_MultiLepCalc->size() > 0 ){
      for(int ind = 0; ind < elMother_id_MultiLepCalc->size(); ind++) {	
    ...
    if ( isMuon==1 && muNumberOfMothers_MultiLepCalc->size() > 0 ){
      for( int ind = 0; ind < muMother_id_MultiLepCalc->size(); ind++ ){
    ...
    if( topPt_TTbarMassCalc->size() == nGenTop && genleptonCount == 1 && !is_genMissingDaughter ){
      ...
      for(unsigned int nTop = 0; nTop < topPt_TTbarMassCalc->size(); nTop++){
        ...
        tempGenWd1.SetPtEtaPhiE(
          topWPt_TTbarMassCalc->at(nTop*2),
          topWEta_TTbarMassCalc->at(nTop*2),
          topWPhi_TTbarMassCalc->at(nTop*2),
          topWEnergy_TTbarMassCalc->at(nTop*2)
        );	
        tempGenWd2.SetPtEtaPhiE(
          topWPt_TTbarMassCalc->at(nTop*2+1),
          topWEta_TTbarMassCalc->at(nTop*2+1),
          topWPhi_TTbarMassCalc->at(nTop*2+1),
          topWEnergy_TTbarMassCalc->at(nTop*2+1)
        );	        
        tempGenb.SetPtEtaPhiE(
          topbPt_TTbarMassCalc->at(nTop),
          topbEta_TTbarMassCalc->at(nTop),
          topbPhi_TTbarMassCalc->at(nTop),
          topbEnergy_TTbarMassCalc->at(nTop)
        );
        if((tempGenWd1.Pt()>30 && tempGenWd2.Pt()>30 && tempGenb.Pt()>30) && (tempGenWd1.Eta()<2.4 && tempGenWd2.Eta()<2.4 && tempGenb.Eta()<2.4)){
          num_genJets_passCuts++;
        }
      }        
    }
...
// build centrality 
    ...
    for (unsigned int ind = 0; ind < v_pair_jet_CSV.size(); ind++){
      if (v_pair_jet_CSV[ind].second>0) totalCSV += v_pair_jet_CSV[ind].second;
    }
...
// BDT trijet_i (leading 4) built from HOTTaggerCalc 
    ...
    for (unsigned int i = 0; i < topDiscriminator_HOTTaggerCalc->size(); i++ ){
      ...
      else if (topDiscriminator_HOTTaggerCalc->at(i) > topBDT2) {
        topBDT4 = topBDT3;
        topBDT3 = topBDT2;
        topBDT2 = topDiscriminator_HOTTaggerCalc->at(i);
      }
      else if (topDiscriminator_HOTTaggerCalc->at(i) > topBDT3) {
        topBDT4 = topBDT3;
        topBDT3 = topDiscriminator_HOTTaggerCalc->at(i);
      }
      else if (topDiscriminator_HOTTaggerCalc->at(i) > topBDT4) {
        topBDT4 = topDiscriminator_HOTTaggerCalc->at(i);
      }
    }
...
////////////////////////////// trijet built from HOT
    ...
    for ( unsigned int i = 0; i < topDiscriminator_HOTTaggerCalc->size(); i++ ){
      ...
      if( topDiscriminator_HOTTaggerCalc->at(i) > BestTop_Discriminator){
        BestTop_Discriminator = topDiscriminator_HOTTaggerCalc->at(i);
    ...
    for( unsigned int i = 0; i < theJetPt_JetSubCalc->size(); i++ ){
      ...
      if( curr_CSV > NoTop_Jet1_CSV ){
        ...
        NoTop_Jet1_Pt = theJetPt_JetSubCalc->at(i);
        NoTop_Jet1_Eta = theJetEta_JetSubCalc->at(i);
        NoTop_Jet1_Phi = theJetPhi_JetSubCalc->at(i);
        NoTop_Jet1_Energy = theJetEnergy_JetSubCalc->at(i);
      }
    }
