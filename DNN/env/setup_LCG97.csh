...
#---First the compiler
if ( -e /cvmfs/sft.cern.ch/lcg/contrib/gcc/8.3.0/x86_64-centos7/setup.csh ) then
    source /cvmfs/sft.cern.ch/lcg/contrib/gcc/8.3.0/x86_64-centos7/setup.csh
endif
...
if ( -e $thisdir/bin/R && "$FC" != "" ) then
    unsetenv R_HOME
    setenv RHOME `echo "cat(Sys.getenv('R_HOME'))" | $thisdir/bin/R --vanilla --slave`
    if ($?ROOT_INCLUDE_PATH) then
    ...
    setenv LD_LIBRARY_PATH ${LD_LIBRARY_PATH}:`echo "cat(find.package('readr'))" | $thisdir/bin/R --vanilla --slave`/rcon
endif
