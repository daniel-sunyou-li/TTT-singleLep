...
#   Generated: Wed Mar 25 18:32:53 2020
...
#  First the compiler
if [ "$COMPILER" != "native" ] && [ -e /cvmfs/sft.cern.ch/lcg/contrib/gcc/8.3.0/x86_64-centos7/setup.sh ]; then
    source /cvmfs/sft.cern.ch/lcg/contrib/gcc/8.3.0/x86_64-centos7/setup.sh
fi

#  LCG version
LCG_VERSION=; export LCG_VERSION
...
PYTHON_VER=`python -c "import sys; print('python{0}.{1}'.format(*sys.version_info))"`
PY_PATHS=${thisdir}/lib/$PYTHON_VER/site-packages
...
if [ -x $thisdir/bin/R ] && [ -n "$FC" ]; then
    unset R_HOME; RHOME=`echo "cat(Sys.getenv('R_HOME'))" | $thisdir/bin/R --vanilla --slave`; export RHOME
    if [ -z "${ROOT_INCLUDE_PATH}" ]; then
    ...
    LD_LIBRARY_PATH=${LD_LIBRARY_PATH}:`echo "cat(find.package('readr'))" | $thisdir/bin/R --vanilla --slave`/rcon
