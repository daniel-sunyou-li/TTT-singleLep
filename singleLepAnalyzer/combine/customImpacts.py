...
for i in range( len( data['params'] ) ):
   thisname_len = len( data['params'][i]['name'] )
   max_label_length = max( [ thisname_len, max_label_length ] )
...
for page in xrange(n):
    ...
    for p in xrange(n_params):
        ...
        padding = max_label_length + 5 
        label_name = thisname + "    " + str(param_idx)
        param_idx += 1
        h_pulls.GetYaxis().SetBinLabel(i + 1, ('#color[%i]{%s}'% (col, label_name)))
    ...
    h_pulls.GetYaxis().SetLabelSize(0.020)
    ...
    if max_impact > 50: max_impact = 50
    h_impacts = ROOT.TH2F(
