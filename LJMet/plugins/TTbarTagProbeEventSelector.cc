#include "TTT-singleLep/LJMet/interface/TTbarTagProbeEventSelector.h"
...
void TTbarTagProbeEventSelector::BeginJob( const edm::ParameterSet& iConfig, edm::ConsumesCollector && iC)
{

    std::cout << mLegend << "initializing TTbarTagProbeEventSelector selection" << std::endl;
    ...
    //nLepton
    minLooseMuons_cut      = selectorConfig.getParameter<bool>("minLooseMuons_cut");
    minLooseMuons          = selectorConfig.getParameter<int>("minLooseMuons");
    maxLooseMuons_cut      = selectorConfig.getParameter<bool>("maxLooseMuons_cut");
    maxLooseMuons          = selectorConfig.getParameter<int>("maxLooseMuons");
    minLooseElectrons_cut      = selectorConfig.getParameter<bool>("minLooseElectrons_cut");
    minLooseElectrons          = selectorConfig.getParameter<int>("minLooseElectrons");
    maxLooseElectrons_cut      = selectorConfig.getParameter<bool>("maxLooseElectrons_cut");
    maxLooseElectrons          = selectorConfig.getParameter<int>("maxLooseElectrons");
    minLeptons_cut           = selectorConfig.getParameter<bool>("minLeptons_cut");
    minLeptons               = selectorConfig.getParameter<int>("minLeptons");
    maxMuons_cut           = selectorConfig.getParameter<bool>("maxMuons_cut");
    maxMuons               = selectorConfig.getParameter<int>("maxMuons");
    maxElectrons_cut           = selectorConfig.getParameter<bool>("maxElectrons_cut");
    maxElectrons               = selectorConfig.getParameter<int>("maxElectrons");
    ...
    push_back("Min Loose Muons");
    push_back("Max Loose Muons");
    push_back("Min Loose Electrons");
    push_back("Max Loose Electrons");
    push_back("Min Leptons");
    push_back("Max Muons");
    push_back("Max Electrons");
    ...
    set("Min Loose Muons",minLooseMuons_cut);
    set("Max Loose Muons",maxLooseMuons_cut);
    set("Min Loose Electrons",minLooseElectrons_cut);
    set("Max Loose Electrons",maxLooseElectrons_cut);
    set("Min Leptons",minLeptons_cut);
    set("Max Muons",maxMuons_cut);
    set("Max Electrons",maxElectrons_cut);
    if (jet_cuts){
...
bool TTbarTagProbeEventSelector::operator()( edm::Event const & event, pat::strbitset & ret) // EVENT SELECTOR!
{
  if(debug)std::cout << "Processing Event in TTbarTagProbeEventSelector::operator()" << std::endl;
...
void TTbarTagProbeEventSelector::AnalyzeEvent( edm::EventBase const & event, LjmetEventContent & ec )
{
  //
...
void TTbarTagProbeEventSelector::EndJob()
{
}

bool TTbarTagProbeEventSelector::TriggerSelection(edm::Event const & event)
{
...
bool TTbarTagProbeEventSelector::PVSelection(edm::Event const & event)
{
...
bool TTbarTagProbeEventSelector::METfilter(edm::Event const & event)
{
...
void TTbarTagProbeEventSelector::MuonSelection(edm::Event const & event)
{
	...
	for (std::vector<pat::Muon>::const_iterator _imu = muonsHandle->begin(); _imu != muonsHandle->end(); _imu++){
	  ...
	  if ( muon_cuts ) {
	    ...
	    //check loose first
	    //For this tag-and-probe purpose, "loose" = "anything" within pT and eta
	    while(1){
	      ...
	      // if (_imu->globalTrack().isNonnull() && _imu->globalTrack().isAvailable() &&
	      // 	  _imu->innerTrack().isNonnull()  && _imu->innerTrack().isAvailable()){ }
	      // else break;
	    ...
	    //check tight
	    //For this tag-and-probe purpose, "tight" = "tightest" around, not analysis
	    while(1){
...
void TTbarTagProbeEventSelector::ElectronSelection(edm::Event const & event)
{
	...
	for (std::vector<pat::Electron>::const_iterator _iel = electronsHandle->begin(); _iel != electronsHandle->end(); _iel++){
	  ...
	  if ( electron_cuts ) {
	    ...
	    //check loose first
	    //loose is nothing except pt and eta and gap
	    while(1){
...
bool TTbarTagProbeEventSelector::LeptonsSelection(edm::Event const & event, pat::strbitset & ret)
{
	bool pass = false;
	...
	unsigned int nMuons = vSelMuons.size();
	unsigned int nLooseMuons = vSelLooseMuons.size();
	unsigned int nElectrons = vSelElectrons.size();
	unsigned int nLooseElectrons = vSelLooseElectrons.size();
	unsigned int nLeps      = vSelMuons.size() + vSelElectrons.size();
	unsigned int nLooseLeps = vSelLooseMuons.size() + vSelLooseElectrons.size();

	bool pass_minLooseMuons = false;
	bool pass_maxLooseMuons = false;
	bool pass_minLooseElectrons = false;
	bool pass_maxLooseElectrons = false;
	bool pass_minLeptons      = false;
	bool pass_maxMuons      = false;
	bool pass_maxElectrons      = false;

	// minimum 2 loose, 1 of each flavor
	if(considerCut("Min Loose Muons")){
		if ( nLooseMuons >= (unsigned int)minLooseMuons){
			pass_minLooseMuons = true;
			passCut(ret, "Min Loose Muons");
			if(debug)std::cout << "\t\t\t" << "pass_minLooseMuons"<<std::endl;
		}
		else{
			return false;
		}
	...
	// maximum 2 loose, 1 of each flavor
	if(considerCut("Max Loose Muons")){
		if ( nLooseMuons <= (unsigned int)maxLooseMuons){
			pass_maxLooseMuons = true;
			passCut(ret, "Max Loose Muons");
			if(debug)std::cout << "\t\t\t" << "pass_maxLooseMuons"<<std::endl;
		}
		else{
			return false;
		}
	...
	if(considerCut("Min Loose Electrons")){
		if ( nLooseElectrons >= (unsigned int)minLooseElectrons){
			pass_minLooseElectrons = true;
			passCut(ret, "Min Loose Electrons");
			if(debug)std::cout << "\t\t\t" << "pass_minLooseElectrons"<<std::endl;
		}
		else{
			return false;
		}
	...
	if(considerCut("Max Loose Electrons")){
		if ( nLooseElectrons <= (unsigned int)maxLooseElectrons){
			pass_maxLooseElectrons = true;
			passCut(ret, "Max Loose Electrons");
			if(debug)std::cout << "\t\t\t" << "pass_maxLooseElectrons"<<std::endl;
		}
		else{
			return false;
		}
	...
	// Maximum 2 tight, 1 of each
	if(considerCut("Max Muons")){
		if ( nMuons <= (unsigned int)maxMuons){
			pass_maxMuons = true;
			passCut(ret, "Max Muons");
			if(debug)std::cout << "\t\t\t" << "pass_maxMuons"<<std::endl;
		}
		else{
			return false;
		}
	...
	if(considerCut("Max Electrons")){
		if ( nElectrons <= (unsigned int)maxElectrons){
			pass_maxElectrons = true;
			passCut(ret, "Max Electrons");
			if(debug)std::cout << "\t\t\t" << "pass_maxElectrons"<<std::endl;
		}
		else{
			return false;
		}
	...
	if(ignoreCut("Min Loose Muons")) pass_minLooseMuons = true;
	if(ignoreCut("Max Loose Muons")) pass_maxLooseMuons = true;
	if(ignoreCut("Min Loose Electrons")) pass_minLooseElectrons = true;
	if(ignoreCut("Max Loose Electrons")) pass_maxLooseElectrons = true;
	if(ignoreCut("Min Leptons")) pass_minLeptons = true;
	if(ignoreCut("Max Muons")) pass_maxMuons = true;	
	if(ignoreCut("Max Electrons")) pass_maxElectrons = true;	

	if(pass_minLooseMuons && pass_maxLooseMuons && pass_minLooseElectrons && pass_maxLooseElectrons && pass_minLeptons && pass_maxMuons && pass_maxElectrons) pass=true;


	return pass;
}

bool TTbarTagProbeEventSelector::JetSelection(edm::Event const & event, pat::strbitset & ret)
{
...
void TTbarTagProbeEventSelector::AK8JetSelection(edm::Event const & event)
{
...
bool TTbarTagProbeEventSelector::METSelection(edm::Event const & event)
{
...
/*
bool TTbarTagProbeEventSelector::EXAMPLESelection(edm::Event const & event)
{
	bool pass = false;
	//
